<!--
 * @Author: your name
 * @Date: 2020-06-01 18:25:51
 * @LastEditTime: 2020-06-01 18:38:17
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: /vue-element-admin/src/views/statistics/fee/compoment/docter.vue
-->
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="接诊人次" align="center">
        <template slot-scope="{row}">
          <span>{{ row.peo_number }}</span>
        </template>
      </el-table-column>
      <el-table-column label="产生收益(药品)" align="center" width="95">
        <template slot-scope="{row}">
          <span>{{ row.sum_drug }}</span>
        </template>
      </el-table-column>
      <el-table-column label="产生收益(检查)" align="center">
        <template slot-scope="{row}">
          <span>{{ row.sum_check }}</span>
        </template>
      </el-table-column>
      <el-table-column label="产生收益(挂号)" align="center">
        <template slot-scope="{row}">
          <span>{{ row.sum_register }}</span>
        </template>
      </el-table-column>
      <el-table-column label="产生收益(其他)" align="center">
        <template slot-scope="{row}">
